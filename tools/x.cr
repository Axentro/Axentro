  ...
  def self.transaction(from_wallet, to_address, iterations = 10)
    File.open("txns.txt", "w") { |f|
      (1..iterations).to_a.each do |_|
        ...
        unsigned_transaction = %Q({"id":"#{id}","action":"send","senders":[{"address":"#{from_address}","public_key":"#{public_key}","amount":100000000,"fee":10000,"signature":"0"}],"recipients":[{"address":"#{to_address}","amount":100000000}],"message":"0","token":"AXNT","prev_hash":"0","timestamp":1601405349647,"scaled":1,"kind":"FAST"})
