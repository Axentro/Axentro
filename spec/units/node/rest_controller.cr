...
describe RESTController do
  ...
  describe "__v1_domain" do
    it "should return the amounts for the specified domain" do
      with_factory do |block_factory, transaction_factory|
        domain = "sushi.sc"
        block_factory.add_block([transaction_factory.make_buy_domain_from_platform(domain, 0_i64)]).add_blocks(2)
  ...
  describe "__v1_domain_transactions" do
    it "should return all transactions for the specified domain" do
      with_factory do |block_factory, transaction_factory|
        transaction = transaction_factory.make_send(100_i64)
        domain = "sushi.sc"
        block_factory.add_block([transaction, transaction_factory.make_buy_domain_from_platform(domain, 0_i64)]).add_blocks(2)
    ...
    it "should paginate default 20 transactions for the specified domain" do
      with_factory do |block_factory, transaction_factory|
        domain = "sushi.sc"
        block_factory.add_block([transaction_factory.make_buy_domain_from_platform(domain, 0_i64)]).add_blocks(100)
        exec_rest_api(block_factory.rest.__v1_domain_transactions(context("/api/v1/domain/#{domain}/transactions"), {domain: domain})) do |result|
          result["status"].to_s.should eq("success")
          transactions = Array(Transaction).from_json(result["result"].to_json)
          transactions.size.should eq(20)
    ...
    it "should paginate transactions for the specified domain" do
      with_factory do |block_factory, transaction_factory|
        domain = "sushi.sc"
        block_factory.add_block([transaction_factory.make_buy_domain_from_platform(domain, 0_i64)]).add_blocks(200)
  ...
  describe "__v1_scars_sales" do
    it "should return the domains for sale" do
      with_factory do |block_factory, transaction_factory|
        domain = "sushi.sc"
        block_factory.add_block([transaction_factory.make_buy_domain_from_platform(domain, 0_i64)]).add_blocks(2).add_block([transaction_factory.make_sell_domain(domain, 1_i64)]).add_blocks(3)
  ...
  describe "__v1_tokens" do
    it "should return a list of existing tokens" do
      with_factory do |block_factory, transaction_factory|
        token = "KINGS"
        block_factory.add_block([transaction_factory.make_create_token(token, 10000_i64)]).add_blocks(3)
