...
describe SlowBlock do
  ...
  describe "#calculate_merkle_tree_root" do
    ...
    it "should calculate merkle tree root when coinbase transaction" do
      ...
      block = SlowBlock.new(2_i64, [coinbase_transaction], 1_u64, "prev_hash", 0_i64, 3_i32)
    ...
    it "should calculate merkle tree root when 2 transactions (first is coinbase)" do
      ...
      block = SlowBlock.new(2_i64, [coinbase_transaction, transaction1], 1_u64, "prev_hash", 0_i64, 3_i32)
  ...
  describe "#valid_as_latest?" do
    it "should return true when valid" do
      with_factory do |block_factory|
        ...
        block = SlowBlock.new(4_i64, [a_coinbase_transaction(1199998747_i64)], a_nonce, prev_hash, timestamp, 0_i32)
    ...
    it "should raise invalid mismatch if there is an index mismatch with the prev block" do
      with_factory do |block_factory|
        ...
        prev_hash = chain[1].to_hash
        timestamp = chain[1].timestamp
        block = SlowBlock.new(98_i64, [a_coinbase_transaction(1199969322_i64)], a_nonce, prev_hash, timestamp, 1_i32)
  ...
  describe "#find_transaction" do
    it "should find a transaction when an matching one exists" do
      coinbase_transaction = a_fixed_coinbase_transaction
      block = SlowBlock.new(2_i64, [coinbase_transaction, a_fixed_signed_transaction], a_nonce, "prev_hash_1", 0_i64, 3_i32)
