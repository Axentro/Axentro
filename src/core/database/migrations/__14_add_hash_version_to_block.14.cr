...
class AddHashVersionToBlock < MG::Base
  def up : String
    <<-SQL
      ALTER TABLE blocks ADD COLUMN hash_version STRING NOT NULL DEFAULT "V1"
    SQL
  end
  ...
  def down : String
    ...
    CREATE TABLE blocks (
      ...
      INSERT INTO blocks SELECT idx, nonce, prev_hash, timestamp, difficulty, address, kind, public_key, signature, hash, version
